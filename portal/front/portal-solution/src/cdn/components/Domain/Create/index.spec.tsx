...
it('renders correctly with type wildcard', () => {
  const tree = renderer.create(
    <DomainCreate type={DomainType.Wildcard} />
  ).toJSON()
  expect(tree).toMatchSnapshot()
})
...
it('renders correctly with type pan', () => {
  const tree = renderer.create(
    <DomainCreate type={DomainType.Pan} pareDomain=".a.com" />
  ).toJSON()
  expect(tree).toMatchSnapshot()
})
...
it('renders correctly with bucket & shouldFixBucket', () => {
  const tree = renderer.create(
    <DomainCreate bucket="test" shouldFixBucket />
  ).toJSON()
  expect(tree).toMatchSnapshot()
})
