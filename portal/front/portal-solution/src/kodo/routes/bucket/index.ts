...
export interface IDetailsOptions<T extends object = object>
  extends IDetailsBaseOptions<T> {
...
export function getDetailsPath<T extends object = object>(
  inject: InjectFunc,
  options: IDetailsOptions<T>
): string {
  const configStore = inject(ConfigStore);
  const { page, path, query, bucketName, anchor } = options;
  return `${configStore.rootPath}/bucket/${page}${path || ""}?${qs.stringify({
    bucketName,
    ...query,
  })}${anchor ? "#" + anchor : ""}`;
}
...
export function getResourceV2Path(
  inject: InjectFunc,
  options: IDetailsBaseOptions<any>
): string {
...
export function getResourceUploadPath(inject: InjectFunc, bucketName: string) {
  return getDetailsPath(inject, {
    page: BucketPage.Resource,
    path: "/upload",
    bucketName,
  });
}

export function getTranscodeStylePath(
  inject: InjectFunc,
  options: IDetailsBaseOptions
) {
  return getDetailsPath(inject, {
    ...options,
    page: BucketPage.TranscodeStyle,
  });
}
