...
@observer
class InternalCDN extends React.Component<IProps & DiDeps & DomainListProps> {
        ...
        render={(disabled) => (
          <Role name={BucketDomainRole.BindCDNDomainEntry}>
            <Button
              ...
              disabled={disabled}
              icon="plus"
              onClick={() => this.handleCreateDomain(visible!)}
            >
              绑定域名
            </Button>
          </Role>
...
export default function CDN(props: IProps & DomainListProps) {
  return (
    <Inject
      render={({ inject }) => <InternalCDN {...props} inject={inject} />}
    />
  );
}
