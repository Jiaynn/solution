...
import { Icon, Select, Alert, Tooltip, Button } from 'react-icecream/lib'
...
class InternalHeaderInfo extends React.Component<IProps & DiDeps> {
    ...
    return (
      <Tooltip title={domain}>
        <div className={styles.defaultDomain}>
          ...
          <div className={styles.defaultTag}>默认</div>
        </div>
      </Tooltip>
    )
  }
  ...
  get CDNDomains() {
    return this.domainStore.getCDNAvailableDomainListByBucketName(this.props.bucketName)
  }

  @computed
  get sourceDomains() {
    return this.domainStore.getSourceDomainListByBucketName(this.props.bucketName)
  }
  ...
  get CDNOptGroupView() {
    ...
    const isDefaultDomainCDNType = this.isDefaultDomainAvailable
      && this.defaultDomain!.domainType === DomainSourceType.CDN

    const needInsertDefaultDomain = isDefaultDomainCDNType
      ...
      <Select.OptGroup label="CDN 域名">
        {
          this.CDNDomains.map(domain => {
            const isDefaultDomain = this.defaultDomain && this.defaultDomain.domain === domain.name

            return (
              <Select.Option key={domain.name} value={`${DomainSourceType.CDN}:${domain.name}`}>
                {
        ...
        }
        {
          needInsertDefaultDomain && (
            <Select.Option value={`${this.defaultDomain!.domainType}:${this.defaultDomain!.domain}`}>
  ...
  get sourceOptGroupView() {
    ...
    if (featureStore.isDisabled('KODO.KODO_SOURCE_DOMAIN')) {
      return null
    }

    const isDefaultDomainSourceType = this.isDefaultDomainAvailable
      && this.defaultDomain!.domainType === DomainSourceType.Source

    const needInsertDefaultDomain = isDefaultDomainSourceType
      && !this.sourceDomains.find(domainInfo => domainInfo.domain === this.defaultDomain!.domain)
      ...
      <Select.OptGroup label="源站域名">
        {
          this.sourceDomains.map(domainInfo => {
            const isDefaultDomain = this.defaultDomain && this.defaultDomain.domain === domainInfo.domain

            return (
              <Select.Option key={domainInfo.domain} value={`${DomainSourceType.Source}:${domainInfo.domain}`}>
                {
  ...
  @computed
  get sharedView() {
    return this.isShared && this.defaultDomain && this.defaultDomain.isAvailable && (
      <Select.Option value={`${this.defaultDomain.domainType}:${this.defaultDomain.domain}`}>
  ...
  get notExistDomainWarningView() {
    ...
    if (
      (this.isShared != null && !this.isShared)
      && store.hasInitDomain
      && !isHasBaseUrl
      && this.bucketInfo && isDomainAvailable(this.props.inject, this.bucketInfo.region)
    ) {
      return (
        <p>
          没有外链域名时，无法预览、下载、复制外链，
          <Link to={getDomainPath(this.props.inject, { bucketName })}>请绑定一个自定义域名</Link>。
        </p>
      )
    }
  ...
  get storageStatisticsView() {
    const content = Object.keys(this.bucketStorageInfo).sort().map(type => {
      const currentTypeStorageStatistics = this.bucketStorageInfo[type as unknown as StorageType]
      const { fileCount, storageSize } = currentTypeStorageStatistics || { fileCount: 0, storageSize: 0 }

      if (fileCount === 0 && storageSize === 0) return null

      return (
        <span key={type} style={{ display: 'inline-block' }}>
          <span className={styles.storageName}>{storageTypeTextMap[type]}：</span>
          <Icon type="folder" />
          ...
          <span className={styles.gapSpan}>
            共 {Number.isFinite(storageSize) ? humanizeStorageSize(storageSize!) : '--'} 存储量
          </span>
        </span>
      )
  ...
  get domainSelectView() {
    if (!this.bucketInfo || !isDomainEnabled(this.props.inject, this.bucketInfo.region)) {
      return
    }
    ...
    return (
      <div className={styles.defaultDomain}>
        <div>
          <label>外链域名：</label>
          <Select
            loading={this.domainStore.isLoading}
          ...
          <Auth
            ...
            render={disabled => (
              <Button
                ...
                onClick={store.setDefaultDomain}
                disabled={disabled || this.isShared || !store.baseUrl}
              >
                保存为默认域名
              </Button>
            )}
...
export default function HeaderInfo(props: IProps) {
  return (
    <Inject render={({ inject }) => (
      <InternalHeaderInfo {...props} inject={inject} />
    )} />
  )
}
