...
export function humanizeChargeType(type: ChargeType, bandwidthType?: SubChargeType) {
  // 只有带宽计费才需要"子计费类型"
...
export function transformUsageForEdit(val: number, chargeType: ChargeType): string {
  const result = chargeType === ChargeType.Bandwidth
    ? NP.divide(val, 1000 ** 2)
    : NP.divide(val, 1024 ** 3)
  return result.toFixed(4)
}

export function transformUsageForSave(val: number, chargeType: ChargeType): number {
  const result = chargeType === ChargeType.Bandwidth
    ? NP.times(val, 1000 ** 2)
    : NP.times(val, 1024 ** 3)
